...
namespace NWH.VehiclePhysics2.Powertrain
{
    ...
    public partial class ClutchComponent : PowertrainComponent
    {
        ...
        /// 
        /// 
        /// field as the clutch can transfer [clutchEngagement * slipTorque] Nm
        ...
        public override float QueryAngularVelocity(float angularVelocity, float dt)
        {
            ...
            else if (controlType == ClutchControlType.UserInput)
            {
                ...
                // For UserInput mode, ensure minimum engagement when throttle is applied
                // This allows the car to move with throttle alone while still requiring clutch for gear changes
                float throttleInput = vehicleController.input.InputSwappedThrottle;
                if (throttleInput > 0.1f && clutchInput < 0.3f)
                {
                    // Apply minimum clutch engagement when throttle is pressed
                    // This simulates automatic-like behavior while maintaining manual clutch control
                    clutchInput = Mathf.Max(clutchInput, 0.3f * throttleInput);
                }
            }
        ...
        public override float ForwardStep(float torque, float inertiaSum, float dt)
        {
            ...
            // For UserInput mode, ensure minimum engagement when throttle is applied
            if (controlType == ClutchControlType.UserInput)
            {
                float throttleInput = vehicleController.input.InputSwappedThrottle;
                if (throttleInput > 0.1f && _clutchEngagement < 0.3f)
                {
                    // Apply minimum clutch engagement when throttle is pressed
                    _clutchEngagement = Mathf.Max(_clutchEngagement, 0.3f * throttleInput);
                }
            }
