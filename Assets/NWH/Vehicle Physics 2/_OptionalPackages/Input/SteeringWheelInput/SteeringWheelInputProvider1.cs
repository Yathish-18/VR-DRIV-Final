...
{
    /// <summary>
    /// Handles input from steering wheels such as Logitech, Thrustmaster, etc.
    ...
    public class SteeringWheelInputProvider1 : VehicleInputProviderBase
    {
        ...
        // ENHANCED: Safety setting like WheelInitialiser
        [Header("🛡️ SAFETY SETTINGS")]
        [Tooltip("Skip shutdown in Editor to prevent crashes")]
        public bool skipShutdownInEditor = true;
        ...
        {
            Debug.Log("[SteeringWheelInputProvider] 🔄 RESETTING STATIC VARIABLES");
            activeInstances = 0;
        ...
        static void OnPlayModeStateChanged(PlayModeStateChange state)
        {
            ...
            {
                Debug.Log("[SteeringWheelInputProvider] 🔄 Play Mode exiting - resetting static variables");
                activeInstances = 0;
                globalSDKActive = false;
        ...
        public override void Awake()
        {
            ...
            {
                Debug.LogWarning("[SteeringWheelInputProvider] ⚠️ Static variables not reset! Manually resetting...");
                activeInstances = 0;
                globalSDKActive = false;
                staticVariablesReset = true;
            }
            ...
            activeInstances++;
            Debug.Log($"[SteeringWheelInputProvider] Awake - Instance: {GetInstanceID()} | Active instances: {activeInstances}");

            // SAFETY CHECK: Prevent instance overflow
            if (activeInstances > 100)
            {
                Debug.LogError($"[SteeringWheelInputProvider] 🚨 CRITICAL: Instance count too high ({activeInstances})! This indicates a static variable reset issue.");
                Debug.LogError("[SteeringWheelInputProvider] 💡 SOLUTION: Check Project Settings > Editor > Enter Play Mode Options");
                Debug.LogError("[SteeringWheelInputProvider] 💡 Either disable 'Enter Play Mode Options' OR ensure Domain Reload is enabled");
                // Emergency reset
                activeInstances = 1;
            }
        ...
        // ENHANCED: Smart SDK initialization (exactly like WheelInitialiser)
        void InitializeSDK()
        {
            Debug.Log("[SteeringWheelInputProvider] Starting SDK initialization process...");

            // First check if SDK is already active (from another component like LogitechSteeringWheel)
            if (TestExistingSDK())
            {
                Debug.Log("[SteeringWheelInputProvider] ✅ Found active SDK! Using existing initialization.");
                Debug.Log("[SteeringWheelInputProvider] Likely initialized by LogitechSteeringWheel or another component");
                sdkInitialized = true;
            ...
            if (!globalSDKActive)
            {
                Debug.Log("[SteeringWheelInputProvider] No existing SDK found, initializing our own...");
                ...
                if (initResult)
                {
                    ...
                    globalSDKActive = true;
                    Debug.Log("[SteeringWheelInputProvider] ✅ Successfully initialized our own SDK");
                }
                else
                {
                    Debug.LogError("[SteeringWheelInputProvider] Failed to initialize Logitech Steering SDK");
                }
            }
            else
            {
                Debug.Log("[SteeringWheelInputProvider] SDK already initialized globally, skipping initialization");
                sdkInitialized = true;
                usingExistingSDK = true;
            }
        }

        // ENHANCED: Test for existing SDK (exactly like WheelInitialiser)
        bool TestExistingSDK()
        {
            Debug.Log("[SteeringWheelInputProvider] Testing for existing SDK activity...");
            try
            {
                bool updateResult = LogitechGSDK.LogiUpdate();
                Debug.Log($"[SteeringWheelInputProvider] LogiUpdate test: {updateResult}");
                if (updateResult)
                {
                    // Try to check if any device is connected as an additional test
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        {
                            anyConnected = true;
                            Debug.Log($"[SteeringWheelInputProvider] Device {i} is connected");
                            break;
                        }
                    }
                    Debug.Log($"[SteeringWheelInputProvider] Connection test - Any Connected: {anyConnected}");
                    return true; // SDK is active even if no wheel is connected
                }
            }
            catch (System.Exception e)
            {
                Debug.Log($"[SteeringWheelInputProvider] SDK test exception: {e.Message}");
            }
            Debug.Log("[SteeringWheelInputProvider] No active SDK detected");
            return false;
        }
        ...
        // ENHANCED: Safe shutdown with proper instance management (EXACTLY like WheelInitialiser)
        void SafeShutdown()
        {
            activeInstances = Mathf.Max(0, activeInstances - 1); // Prevent negative counts
            Debug.Log($"[SteeringWheelInputProvider] SafeShutdown - Instances remaining: {activeInstances} | Using existing SDK: {usingExistingSDK}");
            ...
            if (usingExistingSDK)
            {
                Debug.Log("[SteeringWheelInputProvider] Skipping shutdown - using existing SDK from another component");
                return;
            }

            // Only shutdown if we initialized it ourselves and we're the last instance
            if (activeInstances <= 0 && sdkInitialized && !usingExistingSDK)
            {
                bool shouldShutdown = true;

#if UNITY_EDITOR
                if (skipShutdownInEditor)
                {
                    Debug.Log("[SteeringWheelInputProvider] Skipping shutdown in Editor to prevent crash");
                ...
                if (shouldShutdown)
                {
                    try
                    {
                        ...
                        Debug.Log($"[SteeringWheelInputProvider] Shutdown successful: {shutdownResult}");
                    }
                    catch (System.Exception e)
                    {
                        Debug.LogError($"[SteeringWheelInputProvider] Shutdown error: {e.Message}");
                    }
                    finally
                    {
        ...
        {
            if (sdkInitialized && deviceIndex >= 0)
            {
                try
                {
                ...
                {
                    Debug.LogWarning($"[SteeringWheelInputProvider] Error stopping forces: {e.Message}");
                }
            }
        }
        ...
        {
            Debug.Log($"[SteeringWheelInputProvider] OnDestroy - Instance: {GetInstanceID()}");
        ...
        void GetWheelInputs()
        {
            ...
            // FIXED CLUTCH LOGIC: In NWH, clutch 0 = fully engaged, clutch 1 = fully disengaged
            // When clutch pedal is NOT pressed (raw input = 0), clutch should be engaged (return 0)
            // When clutch pedal IS pressed (raw input = 1), clutch should be disengaged (return 1)
            float rawClutchInput = GetAxisValue(clutchAxis, _wheelInput, clutchZeroToOne);
            if (flipClutchInput) rawClutchInput = -rawClutchInput;
            if (rawClutchInput < clutchDeadzone) rawClutchInput = 0f;

            // The clutch input should be inverted: no pedal press = engaged clutch (0), full pedal press = disengaged clutch (1)
            _clutchInput = rawClutchInput; // Use the raw input directly - NWH expects 0 = engaged, 1 = disengaged
        ...
        {
            Debug.Log($"[SteeringWheelInputProvider] Static Variables Status:");
            Debug.Log($"[SteeringWheelInputProvider] - activeInstances: {activeInstances}");
            Debug.Log($"[SteeringWheelInputProvider] - globalSDKActive: {globalSDKActive}");
            Debug.Log($"[SteeringWheelInputProvider] - staticVariablesReset: {staticVariablesReset}");
            Debug.Log($"[SteeringWheelInputProvider] - sdkInitialized: {sdkInitialized}");
            Debug.Log($"[SteeringWheelInputProvider] - usingExistingSDK: {usingExistingSDK}");
            Debug.Log($"[SteeringWheelInputProvider] - ownedSDKInitialization: {ownedSDKInitialization}");
        }

        // All the original VehicleInputProviderBase override methods
